...
import type { CloseNotification, ShowNotification, ShowNotificationOptions } from "./useNotification";
...
function Notifications({ state }: NotificationsProps) {
    ...
    return currentNotification ? <Notification {...currentNotification} badge={state.queue.length > 1 ? String(state.queue.length) : null} /> : null;
}
